...
for i, label in enumerate(unique_kmeans_labels):
    plt.scatter(train_x_pca[kmeans_labels == label, 0], train_x_pca[kmeans_labels == label, 1], c=colors[i], label=f'Cluster {label} (kMeans)')
...
for i, label in enumerate(unique_dbscan_labels):
    plt.scatter(train_x_pca[dbscan_labels == label, 0], train_x_pca[dbscan_labels == label, 1], c=colors[i], label=f'Cluster {label} (DBScan)')
